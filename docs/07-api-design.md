...
```json
{
  ...
  "code": "VCS_MERGE_CONFLICT",          // 프론트 분기용 안정 코드
  "context": { "conflictedFiles": ["src/a.ts", "..."] }
}
```

주요 에러 코드: `REPO_NOT_FOUND`, `VCS_AUTH_REQUIRED`(401 + 자격증명 재요청 플로우),
`VCS_MERGE_CONFLICT`, `VCS_WORKING_TREE_DIRTY`, `VCS_NETWORK_ERROR`, `OPERATION_CANCELLED`,
`CAPABILITY_NOT_SUPPORTED`(SVN에 stash 요청 등 — 400)
...
| GET | `/repositories/{id}/diff/working?path=&staged=` | 파일 diff (구조화된 hunk JSON) |

### Commits / History

| Method | Path | 설명 |
|---|---|---|
| POST | `/repositories/{id}/commits` | 커밋 `{ message, amend?, paths?(SVN), keepLock?(SVN) }` → 202 |
| GET | `/repositories/{id}/history` | 커서 페이징 `?cursor=&limit=50&branch=&author=&message=&path=&from=&to=` |
| GET | `/repositories/{id}/revisions/{revId}` | 커밋 상세 (메타 + FileChange[]) |
| GET | `/repositories/{id}/revisions/{revId}/diff?path=` | 특정 커밋의 파일 diff |
| GET | `/repositories/{id}/graph` | GraphRow[] 커서 페이징 (레인 배치 포함) |
...
| POST | `/repositories/{id}/merge` | `{ source, ff?, squash? }` → 202 (충돌 시 이벤트로 CONFLICTED) |
...
| POST | `/credentials` / GET `/credentials` / DELETE `/credentials/{id}` | 자격 증명 (비밀은 OS store로) |
