 ...
 * Every method swallows its own failures: a browser that vanished mid-clone must not make the clone fail.</p>
 */
@Component
public class SseOperationEvents implements OperationEvents
{
	...
	/**
	 * Both events exist to invalidate one repository's caches, so without an id there is nothing for a client to act on. Dropping it beats an NPE: the port
	 * promises these never throw, and work with no repository — a clone into a new directory — is exactly the case that passes null.
	 */
	...
	private static Map<String, Object> errorOf(Operation operation) {
		...
		error.put("code", operation.failure().code());
		error.put("detail", operation.failure().message());
		error.put("context", operation.failure().context());
		return error;
	}
