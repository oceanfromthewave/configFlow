...
public class VcsConfig {
    ...
    /**
     * Turns a remote's host/protocol into the stored credential to present, spanning the
     * reference table and the OS keychain. The Git provider consults this on
     * fetch/pull/push/clone; nothing is presented when no credential is stored.
     */
    @Bean
    public RemoteCredentialResolver remoteCredentialResolver(
            CredentialRefStore credentialRefStore, CredentialStore credentialStore) {
