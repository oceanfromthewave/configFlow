 ...
 * port resolution still happen <em>before</em> submitting: a request that cannot possibly
 * work should be rejected with a 4xx now, not accepted and failed asynchronously a second
 * later.</p>
 */
public final class BranchService {
     ...
     *
     * @throws IllegalArgumentException      if the ref is blank
     * @throws NoSuchElementException        if no repository has the given id
     * @throws UnsupportedOperationException if the provider cannot change branches
     */
    ...
    public Operation createBranch(
            RepositoryId id, String name, String startPoint, boolean checkout) {
        ...
        return queue.submit(id, OperationType.BRANCH_CREATE, context -> {
            context.log(
                    "branch " + branchName + (from == null ? "" : " " + from), ConsoleLevel.CMD);
            context.throwIfCancelled();
    ...
    public Operation deleteBranch(
            RepositoryId id, String name, boolean remote, boolean force) {
        String branchName = requireRef(name, "name");
        VcsAccess.Opened<BranchOperations> opened = access.open(id, BranchOperations.class);
