 ...
 *
 * @param staged     staged changes; always empty for providers without the
 *                   {@code STAGING} capability (e.g. SVN)
 * @param unstaged   unstaged (working copy) changes
 * @param conflicted files currently in conflict
 */
public record WorkingTreeStatus(
        List<FileChange> staged,
        List<FileChange> unstaged,
        List<ConflictedFile> conflicted) {
